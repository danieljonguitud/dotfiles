...
local function notify_if_unfocused(window, workspace_name, new_status, pane_cwd, provider)
	...
	if success then
		...
		if frontmost ~= 'wezterm-gui' then
			...
			local app = (provider and provider.label) or 'LLM Agent'
			window:toast_notification(app .. ' — ' .. notif.title, msg)
			pending_workspace = workspace_name
		end
	end
end

function M.check_transitions(window, status, cwd, provider_by_ws)
	provider_by_ws = provider_by_ws or {}
	for ws, s in pairs(status) do
		...
		if (s == 'idle' or s == 'waiting') and prev == 'working' then
			notify_if_unfocused(window, ws, s, cwd[ws], provider_by_ws[ws])
